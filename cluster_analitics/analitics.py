...
class ReductionDim():
    ...
    def _redImplement(self) -> None:   
        ...
        def encoder(capas, inputShape: int,  finalDim: int = 2):
            ...
            for i,capa in enumerate(list(capas)):
                if i == 0:
                    m.add(Dense(capa, activation='elu', input_shape = (inputShape,)))
                else:
    ...
    def initParams(self, args):
        ...
        if self.reductionN == 'encoder':
            ...
            self.reduction = self.reduction(self.capas, self.inputShape, self.finalDim)
    ...
    def fit(self):
        ...
        if self.reductionN == 'encoder':
            ...
            self.reduction.fit(self.x, self.x, batch_size=self.batch_size, epochs=self.epochs, verbose=1,
                validation_data=(self.x, self.x))

            self.reductionEnco = Model(self.reduction.input, self.reduction.get_layer('bottleneck').output)
    ...
    def predict(self):
        ...
        if self.reductionN == 'encoder':
            ...
            return self.reductionEnco.predict(self.x)  # bottleneck representation
    ...
    def fit_predict(self):
        ...
        if self.reductionN == 'encoder':
            self.reduction.compile(loss='mean_squared_error', optimizer = Adam())
            self.reduction.fit(self.x, self.x, batch_size=self.batch_size, epochs=self.epochs, 
                validation_data=(self.x, self.x))
...
class Analisis():
    ...
    def _modImplement(self)  -> None :
        def  lof(n_neighbors, contamination, novelty):
            return LocalOutlierFactor(n_neighbors=n_neighbors, contamination=contamination, novelty=novelty)
        ...
        def som(sigma, learning_rate, neighborhood_function,n1=2,n2=1):
            return MiniSom(n1,n2, input_len = self.inputShape, sigma=sigma, learning_rate=learning_rate, neighborhood_function=neighborhood_function)
    ...
    def initParams(self, args):
        ...
        if self.modeloN == 'lof':
            ...
            self.modelo= self.modelo(self.n_neighbors, self.contamination, novelty=True)
        ...
        if self.modeloN == 'som':
            ...
            self.modelo= self.modelo(self.sigma, self.learning_rate, self.neighborhood_function)
    ...
    def predict(self) -> np.array:
        ...
        if self.modeloN == 'som':
            quantization_errors = np.linalg.norm(self.modelo.quantization(self.x) - self.x, axis=1)
